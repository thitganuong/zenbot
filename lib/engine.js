      ...
      if (s.my_trades.length) {
        ...
        if (!s.acted_on_stop) {
          if (last_trade.type === 'buy') {
            ...
            /*if(s.period.close < s.last_sell_price && so.isDownTrend == false){//down trend gia down thap hon gia da sell thi mua vao
              stop_signal = 'buy'
              console.log(('\n down mua o gia thap hon buy stop triggered at ' + pct(s.last_trade_worth) + ' trade worth\n').red)
            }else*/
            if (s.buy_stop && s.period.close > s.buy_stop && so.isDownTrend == false) {//uptrend //gia cao hon gia sell mua vao
              stop_signal = 'buy'
