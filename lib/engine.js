  ...
  return function (s) {
    ...
    function executeStop (do_sell_stop) {
      ...
      if (s.my_trades.length) {
        ...
        if (!s.acted_on_stop) {
          if (last_trade.type === 'buy') {
            ...
            //Vu fix for rsi if over 65 cancel buy
              if(so.strategy === 'rsi'){
                if(s.period.rsi < 65 && s.signal !== 'sell'){
                
                  stop_signal = 'buy'
                  s.trend = 'long'
                  s.rsi_high = s.period.rsi
                  console.log(('\nbuy stop triggered at ' + pct(s.last_trade_worth) + ' trade worth1\n at rsi' + s.period.rsi ).red)
                }
      ...
      if (stop_signal) {
        ...
        s.acted_on_stop = true //Haint: set trigger always run after that, dont have to wait trend reset
      }
    }
