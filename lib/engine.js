  ...
  return function (s) {
    ...
    function executeStop (do_sell_stop) {
      ...
      if (s.my_trades.length) {
        ...
        so.last_trade_type = last_trade.type
        ...
        s.markMaxPriceFromBuy = Math.max(s.options.markMaxPriceFromBuy, s.period.close)
        so.diffPrice = (s.options.markMaxPriceFromBuy - s.period.close)/ s.period.close*100
        if (!s.acted_on_stop) {
    ...
    function executeOrder (trade) {
      ...
      if (s.buy_order) {
        if (so.mode === 'live' || trade.price <= Number(s.buy_order.price)) {
          ...
          so.markMaxPriceFromBuy = my_trade.price //mark max price
          delete s.buy_order
          delete s.buy_stop
          delete s.sell_stop
