  ...
  return function (s) {
    ...
    function executeStop (do_sell_stop) {
      ...
      if (s.my_trades.length) {
        ...
        so.lastTradeType = last_trade.type
        ...
        //console.log(('\ns.last_trade_worth triggered at ' + pct(s.last_trade_worth) + ' trade worth\n').red)
        if (!s.acted_on_stop) {
          if (last_trade.type === 'buy') {
            if (do_sell_stop && s.sell_stop && s.period.close < s.sell_stop /*&& s.trend ==='short'*/) {//down trend thap hon gia mua
              stop_signal = 'sell'
              console.log(('\nsell stop triggered at '+ s.trend +' ' + pct(s.last_trade_worth) + ' trade worth\n').red)
            }
            ...
            if (s.buy_stop && s.period.close > s.buy_stop) {//uptrend //gia cao hon gia sell mua vao
              stop_signal = 'buy'
              console.log(('\nup mua o gia cao hon buy stop triggered at ' + pct(s.last_trade_worth) + ' trade worth\n').red)
              //so.isMarkRSI = true
              //console.log(('\nNeed RSI!').red)
            }
            // if(s.period.close < s.last_sell_price && so.isDownTrend == false && s.trend !='oversold'){//down trend gia down thap hon gia da sell thi mua vao
            //   stop_signal = 'buy'
            //   console.log(('\n down mua o gia thap hon buy stop triggered at ' + pct(s.last_trade_worth) + ' trade worth\n').red)
            // }else if (s.buy_stop && s.period.close > s.buy_stop && so.isDownTrend == false) {//uptrend //gia cao hon gia sell mua vao
            //   stop_signal = 'buy'
            //   console.log(('\nup mua o gia cao hon buy stop triggered at ' + pct(s.last_trade_worth) + ' trade worth\n').red)
            //   so.isMarkRSI = true
            //   console.log(('\nNeed RSI!').red)
            // } else if(so.isDownTrend == false && s.last_trade_worth >= 0.03){
            //   stop_signal = 'buy'
            //   console.log(('\nup mua o gia thap hon buy stop triggered at ' + pct(s.last_trade_worth) + ' trade worth\n').red)
            //
            // }
          }
        }
      ...
      if (stop_signal) {
        ...
        s.acted_on_stop = true //Haint: set trigger always run after that, dont have to way trend reset
      }
    }
    ...
    function executeOrder (trade) {
      ...
      if (s.buy_order) {
        if (so.mode === 'live' || trade.price <= Number(s.buy_order.price)) {
          ...
          so.lastBuy = s.last_buy_price
          if (so.overSoldMark){
        	  	so.overSoldPrice = so.lastBuy
        	  	console.log(('\nRSI14 oversold mark  : ' + so.overSoldPrice).red)
        	  	so.overSoldMark = false 
          }
          delete s.buy_order
          delete s.buy_stop
          delete s.sell_stop
          s.acted_on_stop = false //Haint: reset when buy/sell done
          if (!s.acted_on_stop && so.sell_stop_pct) {
            s.sell_stop = n(price).subtract(n(price).multiply(so.sell_stop_pct / 100)).value()
            so.willSellAt = s.sell_stop
          }
    ...
    function writeReport (is_progress, blink_off) {
      ...
      so.currentPrice = s.period.close
      if (s.lookback[0]) {
        var diff = (s.period.close - s.lookback[0].close) / s.lookback[0].close
        so.diff = diff //Haint add vao table de trade short none trend
      ...
      get('lib.rsi')(s, 'rsi_custom', so.rsi_periods_custom) //14
      ...
      if (s.buy_order) {
        ...
        process.stdout.write(z(9, ''+so.keep, ' '))
      }
      ...
      if (orig_capital) {
        ...
        so.currentOverBuyHoldPct = over_buy_hold_pct
          ...
          return function (done) {
					...
					if(!so.keep){
					  ...
					  if (s.signal) {
						executeSignal(s.signal)
						 ...
						 console.log(('\nKeep coin/money, abort executeStop or executeSignal').red)
					}
                }
                writeReport()
            ...
            function withOnPeriod () {
              ...
              if (!s.in_preroll) {
                ...
                if (!so.manual) {
					if(!so.keep){
						  executeStop() //Haint: on true to executeSignal in period!!
						  if (s.signal) {
							executeSignal(s.signal)
							s.signal = null
						  }
